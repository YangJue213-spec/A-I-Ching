...
Each flip transforms the hexagram into a safer one; re-judge after each until no
fault remains and the operation is safe to execute.
...
每次翻爻都把卦象变得更安全；每翻一次重判一次，直到没有裂缝、操作可安全执行。
